...
class SeaTalkEventRouter:
    def __init__(self, seatalk_client: SeaTalkClient) -> None:
        self.automation_workflow = AutomationWorkflow(seatalk_client)
    ...
    def handle_event(self, payload: dict[str, Any]) -> None:
        ...
        try:
            self.automation_workflow.process(payload)
        except Exception:
            logger.exception("automation_workflow failed for event_type=%s", event_type)
