...
2. Configure automation pipeline in `app/workflows/automation/`.
- event routing node
- typing status action
- welcome/action response nodes
- graph edges

3. Confirm unified event router in `app/seatalk/events.py`.
- always run `automation_workflow`
- run `chat_workflow` for message event types
...
5. Worker and backend logs confirm successful forwarding and processing.
6. Production `.env` and Worker vars are set and rotated securely.
...
<!-- AUTO_DOCS:BEGIN -->
_Generated by `scripts/sync_docs.py` on 2026-02-12 02:49 UTC_
