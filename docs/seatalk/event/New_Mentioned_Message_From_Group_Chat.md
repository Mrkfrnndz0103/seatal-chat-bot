...
Starting from 11 June 2024, each event will provide a thread_id to indicate whether the user has mentioned the bot in a thread. (Learn more about threading messages in group chats)
...
The type of the event. It will be "new_mentioned_message_received_from_group_chat" in this case
...
The ID of the app to receive the event notifi
...
- It will return empty if the message sender and the bot do not belong to the same organisation.
...
The type of the message. It will be 'text' in this case
...
- Refer to this document to understand the details on how plain text format is converted if the original message is formatted.
